...
> playMoves [m] (Board X xs os as 0) | elem m as = let xs' = insert m xs in
>						   if wins xs'
>						   then Just (Board O xs' os [] 1)
>						   else Just (Board O xs' os (delete m as) 0)
>				     | otherwise = Nothing
> playMoves [m] (Board O xs os as 0) | elem m as = let os' = insert m os in
>						   if wins os'
>						   then Just (Board X xs os' [] -1)
>						   else Just (Board X xs os' (delete m as) 0)
>				     | otherwise = Nothing

> playMoves (m:ms) (Board X xs os as 0) | elem m as = let xs' = insert m xs in
>						      if wins xs'
>					              then Just (Board O xs' os [] 1)
>					              else playMoves ms (Board O xs' os (delete m as) 0)
>				   	| otherwise = Nothing
>
> playMoves (m:ms) (Board O xs os as 0) | elem m as = let os' = insert m os in
>						 if wins os'
>						 then Just (Board X xs os' [] (-1))
>						 else playMoves ms (Board X xs os' (delete m as) 0)
>				   	| otherwise = Nothing
...
Exercise. Now suppose we instead what two have two users playing
against each other.
