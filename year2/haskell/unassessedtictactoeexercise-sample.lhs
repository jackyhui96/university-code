...
> boardExample :: Board
> boardExample = Board X [0,1] [4] [2,3,5,6,7,8] 0
...
> playMoves (m:ms) board 
>   | m `elem` (allowedMoves board) = playMoves ms (play m board)
>   | otherwise                     = Nothing
...
Answer. Run
*Main> noplays (treeOf boardExample)
425
...
Answer. Run
*Main> optimalOutcome(treeOf boardExample)
1
...
> computerAgainstItself (Fork board []) = 
>   putStrLn("Game over with outcome " 
>          ++ show(outcome board) ++ "\n" ++ show board)
> computerAgainstItself (tree@(Fork board forest)) = do
>   putStrLn(show board)
>   putStrLn("I am thinking...")
>   putStrLn("The optimal outcome is " ++ show(optimalOutcome tree))
>   let myMoves = optimalMoves tree
...
>        if winning board' 
>           then putStrLn(show board' ++ "\nI win.")
>           else computerAgainstItself subtree
...
> userAgainstUser (Fork board []) = 
>   putStrLn("Game over\n" ++ show(board))
> userAgainstUser (tree@(Fork board forest)) = do
>   putStrLn(show board)
>   putStr("Please play: ")
>   hFlush stdout
>   s <- getLine
...
>     Nothing -> do
>       putStrLn "Invalid move. Try again."
>       userAgainstUser (Fork board forest)
>     Just tree -> do
>       if winning board
>          then putStrLn (show board ++ "\nYou win.")
>          else userAgainstUser tree
